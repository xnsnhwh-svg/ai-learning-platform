  ...
  <div class="category-manage-container page-enter">
    ...
    <el-dialog title="分类详情" v-model="viewDialogVisible" width="500px">
      <div class="category-detail" v-if="viewCategoryData">
        ...
        <div class="detail-row"><span class="label">分类级别：</span><span class="value">
          <el-tag :type="viewCategoryData.level === 1 ? 'primary' : 'success'">{{ viewCategoryData.level === 1 ? '一级分类' : '二级分类' }}</el-tag>
        </span></div>
        <div class="detail-row" v-if="viewCategoryData.level === 2"><span class="label">父分类：
        </span><span class="value">{{ getParentCategoryName(viewCategoryData.parentId) }}</span></div>
...
const deleteCategory = async (category) => {
  try {
    await ElMessageBox.confirm(`确定要删除分类 "${category.name}" 吗？`,
        '删除确认', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
