  ...
  <div class="video-manage-container page-enter">
    <div class="search-bar">
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-select
            v-model="searchForm.uploaderType"
            placeholder="上传者类型"
            clearable
            @change="handleSearch"
          >
        ...
        <el-col :span="6">
          ...
          <el-button type="success" @click="showUploadDialog" icon="Plus"
            >上传视频</el-button
          >
        </el-col>
      </el-row>
    ...
    <div class="video-list">
      <el-table :data="videoList" v-loading="loading" stripe>
        ...
        <el-table-column label="上传类型" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.uploaderType === 2 ? 'success' : 'info'">{{
              scope.row.uploaderType === 2 ? "管理员" : "用户投稿"
            }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="scope">
            <el-button
              ...
              >预览</el-button
            >
            <!-- <el-button size="small" @click="editVideo(scope.row)" icon="Edit"
              >编辑</el-button
            > -->
            <el-button
              ...
              size="small"
              type="success"
              @click="auditVideoAction(scope.row, 1)"
              icon="Check"
              >通过</el-button
            >
            <el-button
            ...
            <el-button
              ...
              @click="offlineVideoAction(scope.row)"
            ...
            <el-button
              size="small"
              type="danger"
              @click="deleteVideoAction(scope.row)"
              icon="Delete"
              >删除</el-button
            >
          </template>
    ...
    >
      ...
      >
        ...
        <el-form-item label="上传者名称" prop="uploaderName"
          ><el-input
        ...
        <el-form-item label="封面图片">
          ...
          >
            ...
            <template #tip
              ><div class="el-upload__tip">
                支持JPG、PNG格式，建议尺寸16:9，文件大小不超过2MB
              </div></template
            >
          </el-upload>
        </el-form-item>
...
const getStatusText = (status) =>
  ({ 0: "待审核", 1: "已发布", 2: "已拒绝", 3: "已下架" }[status] || "未知");

const getVideoList = async () => {
  loading.value = true;
  try {
    const res = await getVideosForAdmin({
...
const offlineVideoAction = async (video) => {
  if (video.status === 1) {
    try {
      await ElMessageBox.confirm("确定要下架此视频吗？", "提示", {
    ...
    try {
      await ElMessageBox.confirm("确定要重新上架此视频吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      });
      await onlineVideo(video.id, 1);
      ElMessage.success("视频已重新上架");
      getVideoList();
    } catch (error) {
      if (error !== "cancel") console.error("重新上架失败：",)
    }
  }
};
...
const deleteVideoAction = async (video) => {
  try {
    await ElMessageBox.confirm("确定要删除此视频吗？删除后无法恢复！", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
