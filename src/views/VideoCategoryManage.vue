  ...
  <div class="video-category-manage-container page-enter">
    ...
    <div class="category-list">
      <el-table :data="categoryList" v-loading="loading" stripe row-key="id"
                :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :default-expand-all="false" ref="categoryTableRef">
        ...
        <el-table-column prop="videoCount" label="视频数">
          <template #default="scope">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" :close-on-click-modal="false">
      <el-form :model="categoryForm" :rules="categoryRules" ref="categoryFormRef" label-width="100px">
        ...
        <el-form-item label="分类描述" prop="description"><el-input v-model="categoryForm.description" type="textarea" :rows="3"
                                                                placeholder="请输入分类描述" /></el-form-item>
        <el-form-item label="父级分类" prop="parentId" v-if="!isAddChild">
          <el-tree-select v-model="categoryForm.parentId" :data="categoryTreeOptions" :props="{ label: 'name', value: 'id',
          children: 'children' }" placeholder="请选择父级分类（不选则为顶级分类）" clearable check-strictly :render-after-expand="false" />
        </el-form-item>
...
import { getVideoCategories, getCategoryTree, addVideoCategory, updateVideoCategory,
  toggleVideoCategory, deleteVideoCategory } from '../api/videoCategory'
...
const showAddDialog = () => {
  isEdit.value = false; isAddChild.value = false;
  parentCategory.value = null; resetForm(); dialogVisible.value = true; }
const addChildCategory = (parent) => {
  isEdit.value = false; isAddChild.value = true;
  parentCategory.value = parent; resetForm(); categoryForm.parentId = parent.id; dialogVisible.value = true; }
const editCategory = (category) => {
  isEdit.value = true; isAddChild.value = false;
  parentCategory.value = null;
  Object.assign(categoryForm, { ...category, parentId: category.parentId || null });
...
const toggleCategoryStatus = async (category) => {
  try {
    ...
    await toggleVideoCategory({ id: category.id, status: newStatus })
...
const deleteCategory = async (category) => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除分类"${category.name}"吗？`,
